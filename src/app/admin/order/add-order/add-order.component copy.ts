...
export class AddOrderComponent implements OnInit {
	...
	onQuantityChange(index: number): void {
		const productFormGroup = this.productsArray.at(index) as FormGroup;
		const quantity = productFormGroup.get('quantity').value;
		const price = productFormGroup.get('price').value;
		productFormGroup.get('total').setValue(quantity * price);
	}

	createForm() {
		const formGroup = {};
		this.categoryProductList.forEach((product) => {
			formGroup[product.id] = new FormControl(0);
		});
	...
	ngOnInit(): void {
		this.updateDisplayedData();
		this.categoryProductList.map((product) => {
	...
	calculateQty(id: any, qty: any, i: any, productId: any) {
		this.orderForm.patchValue({ productId: 1 });
		if (id == '0' && qty >= 1) {
